...
export const metadata: Metadata = {
  title: "Jamies - Premium Fashion & Clothing Store",
  description: "Discover premium quality clothing at Jamies. Shop the latest fashion trends, timeless classics, and sustainable styles for men and women.",
  keywords: "fashion, clothing, premium apparel, sustainable fashion, men's clothing, women's clothing, trendy clothes",
  metadataBase: new URL("https://jamies-store.com"),
  alternates: {
    canonical: "https://jamies-store.com"
  },
  openGraph: {
    title: "Jamies - Premium Fashion & Clothing Store",
    description: "Discover premium quality clothing at Jamies. Shop the latest fashion trends and timeless classics.",
    siteName: "Jamies",
    type: "website",
    images: [{
      url: "https://images.pexels.com/photos/5490969/pexels-photo-5490969.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
      alt: "Jamies fashion clothing collection"
    }]
  },
  twitter: {
    ...
    title: "Jamies - Premium Fashion & Clothing Store",
    description: "Discover premium quality clothing at Jamies. Shop the latest fashion trends and timeless classics.",
    images: ["https://images.pexels.com/photos/5490969/pexels-photo-5490969.jpeg?auto=compress&cs=tinysrgb&h=650&w=940"]
  },
  robots: {
    index: true,
    follow: true
...
(function() {
  ...
  const handleMessage = (e) => {
    ...
    if (e.data.type === 'webild-delete-element') {
      const { selector, elementType, sectionId, buttonData, textContent } = e.data.data;
      console.log('[Webild] Delete element:', { selector, elementType, sectionId });
      ...
      try {
        ...
        if (!element) {
          console.error('[Webild] Element not found for deletion');
          return;
        }

        // Handle different element types
        if (elementType === 'Section' || element.hasAttribute('data-section')) {
          // For sections, remove the entire div with data-section attribute
          const sectionElement = element.hasAttribute('data-section')
            ? element
            : element.closest('[data-section]');

          if (sectionElement) {
            console.log('[Webild] Removing section:', sectionElement);
            sectionElement.remove();
          }
        } else if (elementType === 'Image') {
          // For images, replace with empty string (hide the image)
          ...
          // For text, replace with empty string
          element.textContent = '';
        } else if (elementType === 'Button') {
          // For buttons, remove the element
          console.log('[Webild] Removing button:', element);
          element.remove();
        } else {
          // Default: remove the element
          console.log('[Webild] Removing element (default):', element);
          element.remove();
        }

        window.parent.postMessage({
          type: 'webild-element-deleted',
          data: { selector, elementType }
        }, '*');

      } catch (error) {
        console.error('[Webild] Error deleting element:', error);
      }
